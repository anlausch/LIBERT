...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      return tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode, logits=logits, scaffold_fn=scaffold_fn)
...
def multitask_model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):
    ...
    model_fn_downstream = model_fn_builder(bert_config, init_checkpoint, learning_rate,
                                   num_train_steps, num_warmup_steps, use_tpu,
                                   use_one_hot_embeddings)
    ...
    with tf.variable_scope("multitask"):
      ...
      def metric_fn(downstream_eval_metrics, wn_eval_metrics):
        ...
        # Display labels and predictions
        #downstream_logits = tf.contrib.metrics.streaming_concat(downstream_eval_metrics['logits'])
        #downstream_label_ids = tf.contrib.metrics.streaming_concat(downstream_eval_metrics['label_ids'])
        downstream_logits = downstream_eval_metrics['logits']
        downstream_label_ids = downstream_eval_metrics['label_ids']
        # Compute Pearson correlation
        downstream_pearson = tf.contrib.metrics.streaming_pearson_correlation(downstream_logits, downstream_label_ids)

        # Compute MSE
        # mse = tf.metrics.mean(per_example_loss)
        downstream_mse = tf.metrics.mean_squared_error(downstream_logits, downstream_label_ids)

        # Compute Spearman correlation
        size = tf.size(downstream_logits)
        indice_of_ranks_pred = tf.nn.top_k(downstream_logits, k=size)[1]
        indice_of_ranks_label = tf.nn.top_k(downstream_label_ids, k=size)[1]
        rank_pred = tf.nn.top_k(-indice_of_ranks_pred, k=size)[1]
        rank_label = tf.nn.top_k(-indice_of_ranks_label, k=size)[1]
        rank_pred = tf.to_float(rank_pred)
        rank_label = tf.to_float(rank_label)
        downstream_spearman = tf.contrib.metrics.streaming_pearson_correlation(rank_pred, rank_label)
        ...
        if wn_model_variant=="BERT" or wn_model_variant == "BERT_MULTICLASS":
          ...
          return {
            ...
            "downstream_pearson": downstream_pearson,
            "downstream_spearman": downstream_spearman,
