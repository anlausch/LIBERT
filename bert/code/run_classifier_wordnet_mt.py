...
flags.DEFINE_bool("multitask", False,
                     "Whether to run the multitask model")
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    elif mode == tf.estimator.ModeKeys.EVAL or mode == ExtendedModeKeys.MULTI_TASK:
      ...
      def metric_fn(per_example_loss, label_ids, logits):
        ...
        # downstream_mcc = tf.to_float(((tp*tn)-(fp*fn))/tf.sqrt((tp+fp)*(tp+fn)*(tn+fp)*(tn+fn)))
        ...
        #mcc = tf.metrics.mean(downstream_mcc)
        mcc = metrics_extension.mcc(label_ids, logits)
        ###
      ...
      elif mode == ExtendedModeKeys.MULTI_TASK:
...
def model_fn_builder_bert_wordnet(bert_config, init_checkpoint, learning_rate,
                                 num_train_steps, num_warmup_steps, use_tpu,
                                 use_one_hot_embeddings, multiclass=False):
  """Returns `model_fn` closure for TPUEstimator."""
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    is_training = (mode == ExtendedModeKeys.TRAIN)
    ...
    if mode == ExtendedModeKeys.TRAIN:
      ...
      return tf.contrib.tpu.TPUEstimatorSpec(
          ...
          train_op=train_op,
          scaffold_fn=scaffold_fn)
    elif mode == ExtendedModeKeys.EVAL or mode == ExtendedModeKeys.MULTI_TASK:
...
def input_fn_builder_multitask(input_files_downstream, input_files_wn,
                            max_seq_length,
                            is_training,
                            drop_remainder,
                            num_cpu_threads=4):
  """Creates an `input_fn` closure to be passed to TPUEstimator."""
  input_function_downstream = file_based_input_fn_builder(input_files_downstream,
                                                  max_seq_length,
                                                  is_training,
                                                  drop_remainder)
  if FLAGS.wn_model_variant == "BERT" or FLAGS.wn_model_variant == "BERT_MULTICLASS":
  ...
  def input_fn(params):
    return {'downstream': input_function_downstream(params),
            'wn': input_function_wordnet(params)}
  return input_fn


def multitask_model_fn_builder(bert_config, num_labels_downstream, init_checkpoint, learning_rate,
                                 num_train_steps, num_warmup_steps, use_tpu,
                                 use_one_hot_embeddings, wn_model_variant):
  ...
  def model_fn(features, labels, mode, params):
    ...
    downstream_features = features["downstream"]
    wn_features = features["wn"]



    model_fn_downstream = model_fn_builder(bert_config, num_labels_downstream, init_checkpoint, learning_rate,
                                   num_train_steps, num_warmup_steps, use_tpu,
                                   use_one_hot_embeddings)
    downstream_loss, downstream_eval_metrics = model_fn_downstream(downstream_features, None, MODE, params)
    ...
    with tf.variable_scope("multitask"):
      loss = downstream_loss + wn_loss

      def metric_fn(downstream_eval_metrics, wn_eval_metrics):
        # downstream task metrics
        downstream_predictions = tf.argmax(downstream_eval_metrics['logits'], axis=-1, output_type=tf.int32)
        downstream_label_ids = downstream_eval_metrics['label_ids']
        downstream_accuracy = tf.metrics.accuracy(downstream_eval_metrics['label_ids'], downstream_predictions)
        downstream_loss = tf.metrics.mean(downstream_eval_metrics['per_example_loss'])

        # TODO: added by anne for coLA?
        fp = tf.metrics.false_positives(labels=downstream_label_ids, predictions=downstream_predictions)
        tp = tf.metrics.true_positives(labels=downstream_label_ids, predictions=downstream_predictions)
        fn = tf.metrics.false_negatives(labels=downstream_label_ids, predictions=downstream_predictions)
        tn = tf.metrics.true_negatives(labels=downstream_label_ids, predictions=downstream_predictions)
        ...
        #       tf.math.multiply(tf.math.add(tp,fp),tf.math.add(tp,fn)),
        #       tf.math.multiply(tf.math.add(tn,fp),tf.math.add(tn,fn)))))
        mcc = metrics_extension.mcc(downstream_label_ids, downstream_predictions)
        #mcc = tf.metrics.mean(downstream_mcc)
        ###

        # wordnet metrics
        wn_per_example_loss = wn_eval_metrics['per_example_loss']
        if wn_model_variant == "BERT" or wn_model_variant == "BERT_MULTICLASS":
      ...
      if mode == ExtendedModeKeys.EVAL:
        return tf.contrib.tpu.TPUEstimatorSpec(
            mode=mode,
            loss=loss,
            eval_metrics=(metric_fn, [downstream_eval_metrics, wn_eval_metrics]),
            scaffold_fn=scaffold_fn)
      ...
      multitask_train_op = optimization.create_optimizer_multitask(standard_loss=downstream_loss, wn_loss=wn_loss, selected_task_id=task_id,
                                                                   wn_upper_bound=wn_upper_bound, init_lr=learning_rate,
                                                                   num_train_steps=num_train_steps, num_warmup_steps=num_warmup_steps,
                                                                   use_tpu=use_tpu)
      tf.summary.scalar('downstream_loss', downstream_loss)
...
def main_run_classifier_multitask(_, config_str, train_batch_size, learning_rate, num_train_epochs):
  tf.logging.set_verbosity(tf.logging.INFO)
  ...
  if FLAGS.do_train:
    ...
    num_train_steps = int(
        len(train_examples) / train_batch_size * num_train_epochs)*2
    num_warmup_steps = int(num_train_steps * FLAGS.warmup_proportion)

  model_fn = multitask_model_fn_builder(
      bert_config=bert_config,
      num_labels_downstream=len(label_list),
      init_checkpoint=FLAGS.init_checkpoint,
      learning_rate=learning_rate,
      num_train_steps=num_train_steps,
      num_warmup_steps=num_warmup_steps,
      use_tpu=FLAGS.use_tpu,
      use_one_hot_embeddings=FLAGS.use_tpu,
      wn_model_variant=FLAGS.wn_model_variant)
  ...
  if FLAGS.do_train:
    ...
    tf.logging.info("  Batch size = %d", train_batch_size)
    tf.logging.info("  Num steps = %d", num_train_steps)
    train_input_fn = input_fn_builder_multitask(input_files_downstream=train_file,
  ...
  if FLAGS.do_eval:
    ...
    eval_input_fn = input_fn_builder_multitask(
        input_files_downstream=eval_file,
        input_files_wn=input_files_wn,
        max_seq_length=FLAGS.max_seq_length,
        is_training=False,
        drop_remainder=eval_drop_remainder)
...
def main(_):
  ...
  for (train_batch_size, learning_rate, train_epochs) in configs:
    ...
    if FLAGS.multitask:
      main_run_classifier_multitask(_, config_str=config_str, train_batch_size=train_batch_size, learning_rate=learning_rate, num_train_epochs=train_epochs)
    else:
